...
class ParquetConverter {
  ...
  async convertColleges() {
    ...
    const tempJsonPath = path.join(process.cwd(), 'data', 'temp_colleges.json');
    fs.writeFileSync(tempJsonPath, JSON.stringify(colleges, null, 2));
  ...
  async convertCourses() {
    ...
    // Write to temporary JSON file
    const tempJsonPath = path.join(process.cwd(), 'data', 'temp_courses.json');
    fs.writeFileSync(tempJsonPath, JSON.stringify(courses, null, 2));
  ...
  async convertCounsellingData() {
    ...
    for (const [year, records] of Object.entries(recordsByYear)) {
      ...
      // Write to temporary JSON file
      const tempJsonPath = path.join(process.cwd(), 'data', `temp_cutoffs_${year}.json`);
      fs.writeFileSync(tempJsonPath, JSON.stringify(records, null, 2));

...
class Integrated5PassOrchestrator:
    ...
    def run_complete_workflow(self):
        ...
        # INTERACTIVE CONTINUATION: If >10 unmatched after PASS 5-AGENTIC, ask to continue
        # Console already imported at top of file
        
        max_agentic_loops = 5  # Safety limit
        agentic_loop_count = 1
        
        while (self.stats.get('pass5_agentic_unresolved', 0) > 10 
               and agentic_loop_count < max_agentic_loops):
            console.print(f"\n[bold yellow]⚠️ {self.stats.get('pass5_agentic_unresolved', 0)} groups still unmatched after PASS 5-AGENTIC[/bold yellow]")
            console.print("[yellow]Would you like to run PASS 5-AGENTIC again on remaining groups?[/yellow]")
            console.print("[dim]  (y/n, default: n, auto-continues in 30s)[/dim]")
            
            # Auto-timeout after 30 seconds (defaults to no)
            import sys
            import select
            ...
            try:
                ...
                if sys.stdin in select.select([sys.stdin], [], [], 30)[0]:
                    ...
                    user_input = 'n'  # Timeout - default to no
                    console.print("[dim]Timeout - continuing without re-run[/dim]")
            except:
            ...
            if user_input == 'y' or user_input == 'yes':
                agentic_loop_count += 1
                console.print(f"\n[bold cyan]🔄 PASS 5-AGENTIC Loop {agentic_loop_count}: Re-processing unmatched groups...[/bold cyan]")
                logger.info(f"PASS 5-AGENTIC Loop {agentic_loop_count}: User requested re-run")
                self._run_agentic_batch()
            else:
                console.print("[yellow]Continuing to next phase...[/yellow]")
                break
        
        if agentic_loop_count >= max_agentic_loops:
            console.print(f"[yellow]⚠️ Max agentic loops ({max_agentic_loops}) reached. Moving on...[/yellow]")
    ...
    def _match_group(self, group: Dict):
        ...
        try:
            ...
            # Try PASS 1 using STREAM FILTERING (NEW: Tier 1 Implementation)
    ...
    def _pass5_fallback(self, college_name: str, state: str, course_type: str,
        ...
        try:
            ...
            if course_type:
                ...
                # Filter master_colleges by stream using recent3_matcher.master_data
    ...
    def _bulk_propagate_results(self):
        ...
        try:
            ...
            SET master_college_id = (
                ...
                WHERE {table}.normalized_state = gmq.normalized_state
                AND {table}.normalized_college_name = gmq.normalized_college_name
                AND COALESCE(NULLIF({table}.normalized_address, ''), 'NO_ADDRESS') = COALESCE(NULLIF(gmq.normalized_address, ''), 'NO_ADDRESS')
                AND {table}.course_type = gmq.sample_course_type
            ),
            college_match_score = (
                SELECT match_score
                FROM group_matching_queue gmq

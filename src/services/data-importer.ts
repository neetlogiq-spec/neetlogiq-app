...
export class DataImporter {
  ...
  private async transformCounsellingData(rawData: RawCounsellingData[]): Promise<ProcessedCounsellingData[]> {
    ...
    // Group by: state → college → year → round → course → quota → source → level
    ...
    for (const record of rawData) {
      ...
      const source = (record as any).master_source_id || (record as any).source_normalized || (record as any).SOURCE || (record as any).Source || '';
      const level = (record as any).master_level_id || (record as any).level_normalized || (record as any).LEVEL || (record as any).Level || '';
      const key = `${record.STATE}|${record['COLLEGE/INSTITUTE']}|${record.YEAR}|${record.ROUND}|${record.COURSE}|${record.QUOTA}|${source}|${level}`;

...
from llm_performance_tracker import get_performance_tracker
...
MODEL_CONFIG = {
    ...
    # Priority 1: QUALITY - Fallback tier 1 (ordered by sub_priority: lower = better)
    "meta-llama/llama-3.3-70b-instruct:free": {
        "max_tokens": 8192,
        "batch_size": 15,
        "timeout": 90,
        "priority": 1,
        "sub_priority": 0,  # Best: 70B params, strong reasoning
    ...
    "google/gemini-2.0-flash-exp:free": {
        ...
        "timeout": 90,
        "priority": 1,
        "sub_priority": 1,  # Excellent: Google's latest Flash model
    ...
    "openai/gpt-oss-20b:free": {
        ...
        "timeout": 60,
        "priority": 1,
        "sub_priority": 2,  # Good: OpenAI quality, smaller
    ...
    "kwaipilot/kat-coder-pro:free": {
        ...
        "timeout": 90,
        "priority": 1,
        "sub_priority": 3,  # Coding focused, less general reasoning
...
# Priority-separated model lists for hybrid retry strategy
P0_MODELS = [m for m in MODEL_CONFIG.keys() if MODEL_CONFIG[m]["priority"] == 0]  # Best reasoning
# P1 models sorted by sub_priority (best reasoning first: Llama 70B > Gemini > GPT OSS > Kat Coder)
P1_MODELS = sorted(
    [m for m in MODEL_CONFIG.keys() if MODEL_CONFIG[m]["priority"] == 1],
    key=lambda m: MODEL_CONFIG[m].get("sub_priority", 99)  # Default to 99 if no sub_priority
)
P2_MODELS = [m for m in MODEL_CONFIG.keys() if MODEL_CONFIG[m]["priority"] == 2]  # Capacity fallback
...
Is this match CORRECT? Analyze step-by-step considering name+address, then respond with JSON."""
    ...
    def _verify_single_record(
        ...
        try:
            ...
            # Record successful call to performance tracker
            ...
            elif "timeout" in error_str.lower() or "503" in error_str:
                error_code = "timeout"
            else:
                error_code = "error"
            
            # Record failed call to performance tracker
            perf_tracker = get_performance_tracker()
            perf_tracker.record_call(
                model=model,
                success=False,
    ...
    def verify_with_super_parallel(
        ...
        def process_batch_with_consensus(batch_idx: int, batch: List[VerificationRecord]) -> Dict[str, List[ModelVote]]:
            """
            Process a single batch with multi-model consensus using HYBRID P0 RETRY + P1 FALLBACK.
            
            Strategy:
            1. Try 3 P0 models first (best reasoning)
            2. If P0 fails → Immediately retry with another P0 model (max 2 retries per slot)
            3. If still missing votes after P0 exhausted → Fall back to P1 models
            
            Returns dict of record_id -> list of ModelVote with source tracking.
            """
            batch_votes: Dict[str, List[ModelVote]] = {r.record_id: [] for r in batch}
            
            # Rotate P0 models for this batch to distribute load
            import random
            p0_models = P0_MODELS.copy()
            random.shuffle(p0_models)  # Shuffle P0 to distribute load across API pools
            p1_models = P1_MODELS.copy()  # Keep P1 in priority order (sorted by sub_priority)
            ...
            # PHASE 1: Try P0 models with immediate retry
            max_p0_attempts = min(len(p0_models), required_votes + 2)  # Try up to 5 P0 models
            
            for attempt in range(max_p0_attempts):
                if votes_collected >= required_votes:
                    break  # Got enough votes
                
                # Pick next untried P0 model
                model = None
                for m in p0_models:
                    if m not in p0_tried:
                        model = m
                        break
                
                if not model:
                    break  # Exhausted P0 models
                ...
                try:
                    ...
                    logger.debug(f"Batch {batch_idx} P0 model {model.split('/')[-1][:15]} failed: {e}")
                    continue  # Immediate retry with next P0 model
            
            # PHASE 2: If still missing votes, fall back to P1 models
            if votes_collected < required_votes:
                missing = required_votes - votes_collected
                logger.debug(f"Batch {batch_idx}: P0 yielded {votes_collected}/{required_votes} votes, trying P1 fallback")
                
                max_p1_attempts = min(len(p1_models), missing + 1)  # Try enough P1 to fill gaps
                
                for attempt in range(max_p1_attempts):
                    if votes_collected >= required_votes:
                        break
                    
                    # Pick next untried P1 model
                    model = None
                    for m in p1_models:
                        if m not in p1_tried:
                            model = m
                            break
                    
                    if not model:
                        break  # Exhausted P1 models
                    ...
                    try:
                        ...
                        logger.debug(f"Batch {batch_idx} P1 model {model.split('/')[-1][:15]} failed: {e}")
                        continue
...
def run_verification(
    ...
    for r in records:
        # PRIORITY: corrected_college_name (from embedding fixer) > seat_college_name
        college_name = (
            r.get('corrected_college_name') or 
            r.get('seat_college_name') or ''
        )
